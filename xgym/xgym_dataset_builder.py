    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        return {
            "train": self._generate_examples(
                tfds.builder_from_directory(root).as_dataset(split="train")
            ),
        }
    ...
    def _generate_examples(self, ds) -> Iterator[Tuple[str, Any]]:
        ...
        def _parse_example(idx, ep):
            ...
            for i, step in enumerate(ep['steps']):
                ...
                task = "put the yellow block on the green block"  # hardcoded for now
                ...
                # the gripper action should be absolute position... proportional to the gripper's range
                action[-1] = (step['observation']['proprio']['position'][-1] + action[-1]) / 850
                # print([round(x,4) for x in action.tolist()])
